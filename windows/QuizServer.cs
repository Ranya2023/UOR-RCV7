...
internal sealed class QuizServer : IDisposable
{
    ...
    public void NewQuestion(string question, List<string> options, int correct, int index, int count)
    {
        lock (_lock)
        {
        ...
        lock (_lock)
        {
            ...
            foreach (var (id, a) in _answers)
            {
                ...
                if (p.LastCorrect)
                {
                    ...
                    p.Last = (int)Math.Round(1000 * (1 - Math.Min(1, secs / 30.0) / 2));   // 1000 fast … 500 slow
                    p.Score += p.Last;
                }
            }
        }
    ...
    private string StateJson(Dictionary<string, string> q)
    {
        ...
        lock (_lock)
        {
            ...
            return JsonSerializer.Serialize(new
            {
                ...
                qi = QIndex, qn = QCount, joined = me != null, name = me?.Name ?? "",
